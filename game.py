...
class SnakeGame:
	...
	def get_state(self):
		...
		sensor.pop(-3)
		s = [angle, self.food.x > self.head.x, self.food.x < self.head.x, self.food.y > self.head.y, self.food.y < self.head.y, 1 / (abs(self.head.x - self.food.x) + abs(self.head.y - self.food.y) + 1e-15)]
		for x, y in sensor:
			dx = self.head.x + x
			dy = self.head.y - y
			p = Point(dx, dy)
