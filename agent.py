...
class DQN(nn.Module):
	...
	def __init__(self, h, w, outputs):
		...
		self.conv1 = nn.Conv1d(2, 1, kernel_size=5, stride=2)
		self.bn1 = nn.BatchNorm2d(1)
		# self.conv2 = nn.Conv1d(16, 1, kernel_size=5, stride=2)
...
def select_action(state):
	...
	if sample > eps_threshold:
		with torch.no_grad():
			...
			print(type(policy_net(state).max(1)[1].view(1, 1)))
	...
	if SnakeGame.NORMAL_GAME:
		...
		for i_episode in range(num_episodes):
			...
			for t in count():
				...
				_, reward, done = env.play_step(action=action_game)
				print(reward)
